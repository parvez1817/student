  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        ...
        const acceptedRes = await fetch(`http://localhost:5000/api/acceptedidcards/user/${registerNumber}`);
        ...
        const rejectedRes = await fetch(`http://localhost:5000/api/rejhistoryids/user/${registerNumber}`);
